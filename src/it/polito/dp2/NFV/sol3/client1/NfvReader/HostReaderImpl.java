...
public class HostReaderImpl extends NamedEntityImpl implements HostReader {
	...
	public HostReaderImpl(HostType h) {
		...
		Set<VirtualNodeType> allocatedNodes=null;
		
		if(h.getVirtualizednodes()!=null && h.getVirtualizednodes().getVirtualizednode()!=null)
			allocatedNodes=new HashSet<>(h.getVirtualizednodes().getVirtualizednode());
		
		if(allocatedNodes!=null && !allocatedNodes.isEmpty()) {	//possible null
